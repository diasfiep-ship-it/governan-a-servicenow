...
import { MessageCircle, X, Send, Loader2, Minimize2, Maximize2, Mic, MicOff, Database } from 'lucide-react';
...
interface ChatPriorizacaoProps {
  chamados: Chamado[];
  isAdmin: boolean;
}
...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/chat-priorizacao`;

export default function ChatPriorizacao({ chamados, isAdmin }: ChatPriorizacaoProps) {
  ...
  // Filter chamados based on user access
  const filteredChamados = useMemo(() => {
    if (isAdmin) {
      // ADM has access to all chamados
      return chamados;
    }

    // For non-admin users, filter by their area roles
    return chamados.filter(chamado => {
      const chamadoAreaNormalized = normalizeAreaName(chamado.area_demandante);
      
      return roles.some(role => {
        // Check if role matches area (case and accent insensitive)
        const roleNormalized = normalizeAreaName(role);
        return chamadoAreaNormalized === roleNormalized;
      });
    });
  }, [chamados, isAdmin, roles]);

  // Prepare context for the API
  const chamadosContext = useMemo(() => {
    return filteredChamados.map(c => ({
      numero: c.numero,
      ...
      selecionado_mes: c.selecionado_mes || false,
    }));
  }, [filteredChamados]);
  ...
  const accessDescription = isAdmin 
    ? 'Acesso total' 
    : `Acesso: ${roles.filter(r => !['ADM', 'ADM_TI', 'TI', 'ESCRITORIO_PROCESSOS'].includes(r)).join(', ') || 'Limitado'}`;
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-muted/50 rounded-t-lg">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-sm">Assistente de Priorização</span>
        </div>
        ...
        <>
          ...
          <ScrollArea className="flex-1 p-4" ref={scrollRef}>
            {messages.length === 0 ? (
              <div className="text-center text-muted-foreground text-sm py-8">
                ...
                <p>Pergunte sobre os chamados e priorização.</p>
                <p className="text-xs mt-4 text-muted-foreground/70">
                  Ex: "Quais são os chamados mais críticos?" ou "Resumo por área"
                </p>
              </div>
            ) : (
          ...
          <form onSubmit={handleSubmit} className="p-3 border-t border-border">
            ...
            <p className="text-[10px] text-muted-foreground mt-2 text-center">
              {accessDescription} • {filteredChamados.length} chamados
            </p>
          </form>
        </>
      )}
